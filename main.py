...
if game_type == 1:
    ...
    record_csv_deck(player_a,'争上游01副牌.csv')
    record_csv_deck(player_b,'争上游02副牌.csv')
...
if game_type == 3:
    ...
    record_csv_deck(player_a,'三人斗地主01副牌.csv')
    record_csv_deck(player_b,'三人斗地主02副牌.csv')
    record_csv_deck(player_c,'三人斗地主03副牌.csv')
    record_csv_deck(player_dumy,'三人斗地主预留牌.csv')
if game_type == 4:
    deal_to_multi_players_remain(deck,8,player_dumy,player_a,player_b,player_c,player_d)
    record_csv_deck(player_a,'四人斗地主01副牌.csv')
    record_csv_deck(player_b,'四人斗地主02副牌.csv')
    record_csv_deck(player_c,'四人斗地主03副牌.csv')
    record_csv_deck(player_d,'四人斗地主04副牌.csv')
    record_csv_deck(player_dumy,'四人斗地主预留牌.csv')
...
if deck_no == 0:
    ...
    if game_type ==3:
        if deck_no >=1 and deck_no<=3:
            ...
            filename = '三人斗地主预留牌.csv'
    if game_type ==4:
        if deck_no >=1 and deck_no<=4:
            filename = '四人斗地主%02d副牌.csv'%(deck_no)
        elif deck_no ==9:
            filename = '四人斗地主预留牌.csv'
    if filename !='no such a file.csv':
        my_deck = []
